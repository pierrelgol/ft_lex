...
pub fn init(inputs: []const u8) BreTokenizer {
    return .{
        .inputs = inputs,
        .index = 0,
        .mode = .in_regex_expr,
...
pub fn lexer(self: *BreTokenizer) Lexer {
    return .{
        .ptr = self,
        .vtable = &.{
            .nextFn = next,
...
fn advance(ctx: *anyopaque, count: usize) void {
    var self: *BreTokenizer = @ptrCast(@alignCast(ctx));
    self.index += count;
}
...
fn advanceInternal(self: *BreTokenizer, count: usize) void {
    self.index += count;
}

fn nextInternal(self: *BreTokenizer) Token {
    const res = self.peekInternal();
    self.advanceInternal(res.bytes);
    return res.token;
}

fn peekInternal(self: *BreTokenizer) TokenResult {
...
fn tokenize(self: *const BreTokenizer) TokenResult {
    if (self.index >= self.inputs.len) {
        return TokenResult.EOF;
    }
    return switch (self.mode) {
        .in_regex_expr => self.tokenizeRegexExprBRE(),
        ...
        .in_intrv_expr => self.tokenizeIntrvExprBRE(),
    };
}

fn tokenizeRegexExprBRE(self: *const BreTokenizer) TokenResult {
    ...
    switch (c) {
        '\\' => {
            ...
            switch (next_c) {
                '(' => return .{ .token = .open_group, .bytes = 2 },
                ')' => return .{ .token = .close_group, .bytes = 2 },
                '{' => return .{ .token = .open_interval_brace, .bytes = 2 },

                '|' => return .{ .token = .alternation, .bytes = 2 },
                '+' => return .{ .token = .one_or_more, .bytes = 2 },
                '?' => return .{ .token = .zero_or_one, .bytes = 2 },
                ...
                else => return .{ .token = .{ .literal = next_c }, .bytes = 2 },
            }
        },
...
fn tokenizeIntrvExprBRE(self: *const BreTokenizer) TokenResult {
    assert(self.mode == .in_intrv_expr);
    const c = self.inputs[self.index];

    switch (c) {
        '\\' => {
            if (self.index + 1 < self.inputs.len and self.inputs[self.index + 1] == '}') {
                return .{ .token = .close_interval_brace, .bytes = 2 };
            } else {
                std.debug.print("Error: Unexpected backslash in interval at {d}\n", .{self.index});
                return TokenResult.EOF;
            }
        },
        ',' => return .{ .token = .interval_comma, .bytes = 1 },
            ...
            if (ascii.isDigit(c)) {
                ...
                const num = std.fmt.parseUnsigned(u32, num_slice, 10) catch |err| {
                    std.debug.print("Error parsing interval number: {s}\n", .{@errorName(err)});
                    return TokenResult.EOF;
                };
                return .{ .token = .{ .interval_number = num }, .bytes = end - self.index };
            } else {
                std.debug.print("Error: Invalid character '{c}' in interval at {d}\n", .{ c, self.index });
                return TokenResult.EOF;
            }
        },
    }
...
fn tokenizePosixExpr(self: *const BreTokenizer) TokenResult {
...
fn tokenizeClassExpr(self: *const BreTokenizer) TokenResult {
    assert(self.mode == .in_class_expr);

    if (self.index + 1 < self.inputs.len and self.inputs[self.index] == ':' and self.inputs[self.index + 1] == ']') {
    ...
    if (end > self.index) {
        return .{ .token = .{ .character_class_name = self.inputs[self.index..end] }, .bytes = end - self.index };
    } else {
        return TokenResult.EOF;
    }
}

fn tokenizeCollaExpr(self: *const BreTokenizer) TokenResult {
    ...
    if (end > self.index) {
        return .{ .token = .{ .collating_element_name = self.inputs[self.index..end] }, .bytes = end - self.index };
    } else {
        return TokenResult.EOF;
    }
}

fn tokenizeEquivExpr(self: *const BreTokenizer) TokenResult {
    ...
    if (end > self.index) {
        return .{ .token = .{ .equivalence_class_name = self.inputs[self.index..end] }, .bytes = end - self.index };
    } else {
        return TokenResult.EOF;
    }
}
...
fn expectTokens(input: []const u8, expected_kinds: []const Token.Kind) !void {
    var lex = BreTokenizer.init(input);
...
test "lexer BRE backslash escape metacharacter" {
    const input: []const u8 = "\\.\\*";
    const expected = [_]Token.Kind{ .literal, .literal };
    try expectTokens(input, &expected);
}

test "lexer BRE backslash escape literal" {
    const input: []const u8 = "\\a";
    const expected = [_]Token.Kind{.literal};
    try expectTokens(input, &expected);
}

test "lexer BRE backslash escape backslash" {
    const input: []const u8 = "\\\\";
    const expected = [_]Token.Kind{.literal};
    try expectTokens(input, &expected);
}

test "lexer BRE literal operators" {
    const input: []const u8 = "a+b|c(d)e{1}";
...
test "lexer BRE bracket with literal hyphen start" {
    const input: []const u8 = "[-abc]";
    const expected = [_]Token.Kind{ .open_bracket, .literal, .literal, .literal, .literal, .close_bracket };
    try expectTokens(input, &expected);
}

test "lexer BRE bracket with literal hyphen end" {
    const input: []const u8 = "[abc-]";
    const expected = [_]Token.Kind{ .open_bracket, .literal, .literal, .literal, .literal, .close_bracket };
    try expectTokens(input, &expected);
}

test "lexer BRE bracket character class" {
...
test "lexer BRE bracket collating element" {
    const input: []const u8 = "[[.a.]b]";
...
test "lexer BRE bracket equivalence class" {
    const input: []const u8 = "[[=a=]b]";
